...
class SelectCar {
    ...
    addEventSelectMark(selectMark) {
        $(selectMark).on('change', () => {
            ...
            let modelHTML = this.modelHTML.clone();
            let parent = $(selectMark).parent();
            if(parent.find(this.modelSelector).length === 0) {
                if($(selectMark).parent().find(this.btnGroupCarDelSelector).length === 0) {
                    $(selectMark).after(modelHTML);
                } else {
                    $(selectMark).parent().after(modelHTML);
                }
                modelHTML.show('slow');
            }
...
const updateCardsCar = function(formFields, typeForm) {
    let data = {};
    let cars = [];
    ...
    $.ajax({
        url: url_path + '/department/taxicards',
        data: data,
        method: 'GET',
        success: function(msg) {
            console.log(msg)
...
$('.params-panel-form').on('change', function(e) {
    ...
    switch(typeForm) {
        case 'cars':
            updateCardsCar(formFields, typeForm);
            break;
    }
});
